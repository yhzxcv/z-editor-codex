...
window.ZEditor.Tree = (function () {
   ...
   *
   * ✎ 和 ✕ **都常驻**，不藏。跳转 / 改参数 / 删除是这一行上平级的三件事，三个入口
   * 得一样看得见 —— 悬停才显形的按钮，用户先得把指针移上去才被告知"这儿能删"。
   * （原先桌面上 ✕ 是 opacity:0，2026-09-22 用户点名去掉；手机上那条"点过才出现"
   * 的问题是同一个毛病，只是更早暴露。）
   *
   * 安静那一档只在有悬停能力的设备上做（css/app.css 里那条 @media (hover: hover)，
   * 静止 .35、悬停或键盘聚焦才实起来）：一行上三个入口全实心的话这棵树读不下去。
   * 触摸屏上两个按钮是**常驻实心** —— 那里没有 hover 反馈来解释"为什么它淡"。
   */
